...
class User {
  ...
  /**
   * Update YouTube OAuth tokens
   */
  static updateYouTubeTokens(userId, accessToken, refreshToken) {
    return new Promise((resolve, reject) => {
      db.run(
        'UPDATE users SET youtube_access_token = ?, youtube_refresh_token = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
        [accessToken, refreshToken, userId],
        function (err) {
          if (err) {
            console.error('Database error in updateYouTubeTokens:', err);
            return reject(err);
          }
          resolve({ id: userId, tokensUpdated: true });
        }
      );
    });
  }
